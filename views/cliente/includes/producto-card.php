...
>
    <div class="aspect-[3/4] mb-6 overflow-hidden bg-surface-container relative">
        ...
        <button type="button" class="absolute top-4 right-4 z-10 transition-colors" data-toggle-favorite
            ...
            aria-label="Añadir a favoritos" aria-pressed="false">
    ...
    <div class="mt-4">
        ...
        <div class="flex flex-wrap gap-2" data-card-size-selector>
            ...
            <button type="button" class="card-size-btn min-w-10 h-10 px-2 flex items-center justify-center font-label-md text-label-md transition-colors <?= $activa ? 'bg-secondary text-on-secondary border border-secondary' : 'border border-outline-variant hover:border-secondary' ?>"><?= htmlspecialchars($talla) ?></button>
            <?php endforeach; ?>
        </div>
    </div>
    <?php if ($tieneColores): ?>
    ...
    <button type="button"
        ...
        data-add-to-cart data-product-talla-from="card"
        data-product-id="<?= $productoId ?>"
        data-product-nombre="<?= htmlspecialchars($producto['nombre'], ENT_QUOTES) ?>"
        data-product-precio="<?= (float) $producto['precio'] ?>"
        data-product-imagen="<?= htmlspecialchars($imagenUrl, ENT_QUOTES) ?>"
